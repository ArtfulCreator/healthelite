...
<title>HealthElite - Coach Information</title>
</head>
<body>
	<center>
				...
				<tr>
					...
					<td style="width: 163px; height: 19px; text-align: center;"
						...
						<div style="text-align: right;height: 25px;">
							<form:form method="post" action="addCoach" name="addCoach">
								<security:authorize access="isAuthenticated()">	
									<c:if test="${not empty traineeInfo && login.equals(traineeInfo.roles.login)}">
										<c:if test="${!coachInfo.isTrainee(traineeInfo)}">
											<input type="submit" value="Add Coach"/>
										</c:if>
										
										<c:if test="${coachInfo.isTrainee(traineeInfo) && coachInfo.isTraineeActive(traineeInfo)}">
											Active Coach
										</c:if>
										<c:if test="${coachInfo.isTrainee(traineeInfo) && !coachInfo.isTraineeActive(traineeInfo)}">
											Pending Activation
							...
							<tbody>
								...
								<tr>
									<td style="vertical-align: top; width: 385px;">${coachInfo.roles.login.getFirstName()} ${coachInfo.roles.login.getLastName()}<br>
									</td>
								...
								<tr>
									<td style="vertical-align: top; width: 358px; height: 118px;">${coachInfo.getSpeciality()}<br>
									</td>
								</tr>
								<tr>
									<td colspan="2" rowspan="1"
										style="vertical-align: top; width: 158px; height: 0px;">${coachInfo.getDescription()}<br>
									</td>
								</tr>
								<tr>
									<td colspan="2" rowspan="1"
